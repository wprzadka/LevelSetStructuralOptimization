...
class LevelSetMethod:
    ...
    def optimize(self, iteration_limit: int):
        ...
        if self.updater_type == LevelSetUpdaterType.RADIAL_BASE_FUNCTIONS:
            ...
            phi = RbfFilterPoints(points_ratio=0.3, points=elems_centers, init_values=init_phi_elems)
        ...
        for i in range(1, iteration_limit):
            ...
            v_function = elems_compliance - lag
            ...
            for _ in range(10):
                phi.update(v_function_filtered, 0.1 / np.max(np.abs(v_function_filtered)))
            ...
            if __debug__ or i == iteration_limit - 1:
                ...
                plot_displ(self.mesh, displacement, density)
                plt.savefig(f'plots/displ{i}')
